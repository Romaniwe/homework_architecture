...
## 6. Физическая схема БД
   ...
   | Таблица | Состав индекса (Primary Key) | Пояснение |
   |---------|-----------------------------|-----------|
   | **chats** | 1) `(user_id, chat_id)` | Получение всех диалогов пользователя с сортировкой по ID |
   | **messages** | 1) `(chat_id, msg_id)` | Получение всех сообщений диалога с сортировкой по времени |
   | **messages_by_user** (MV) | 1) `(user_id, created_at, chat_id, msg_id)` | Получение сообщений пользователя для обратной связи и аналитики |
