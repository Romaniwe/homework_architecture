...
## 4. Локальная балансировка нагрузки
   ...
   ### L4 балансировщик: LVS (linux virtual server) распределяет трафик на L7 балансировщики ( а вот уже ответ от бека пойдет напрямую пользователю, а не на балансироващик обратно ). Weightet least connection смотрит, на список подключений и выбирает менее загруженный. Почему weightet, чтобы загружать побольше более сильные сервера.
   ### L7 балансировщики (Nginx): Занимаются SSL Termination и отправляют на бэкенды по http уже.  
