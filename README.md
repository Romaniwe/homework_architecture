...
- **Feature flags в Redis**: каждая фича (search, thinking, vision) имеет глобальный флаг enabled/disabled, который Chat Service проверяет перед использованием. Управляется автоматически на основе Prometheus метрик через AlertManager.
...
- **Retry с exponential backoff и jitter**: при transient-ошибках retry, но с экспоненциальной задержкой и случайным jitter'ом.
