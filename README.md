...
> Горизонтальное автомасштабирование (HPA) настроено по метрике CPU utilization > 70%. Максимальное число реплик в 2× от базового.
...
> Доминирующая статья — GPU-серверы инференса (474 из 511). Это отражает реальную структуру затрат OpenAI: по оценкам аналитиков, более 80% капитальных расходов приходится на GPU-кластеры.
