...
Высоконагруженная система ChatGPT-аналога обслуживает 193 миллиона пользователей в день и около 90 тысяч RPS в пике. На таких объёмах единичный сбой компонента не должен приводить к отказу всей системы или к потере данных. Стратегия обеспечения надёжности строится на четырёх принципах: (1) каждый компонент имеет резервирование с явной формулой; (2) деплои и shutdown происходят без потери in-flight запросов; (3) при перегрузке система деградирует постепенно, а не отказывает целиком; (4) любая аномалия видна в observability-стеке до того, как её заметит пользователь.

### 9.1 Сводная таблица методов резервирования

| Компонент системы | Метод обеспечения надёжности |
|-------------------|------------------------------|
| **Cloudflare (Edge)** | Anycast — IP анонсируется из сотен дата-центров одновременно через BGP; падение одного PoP'а прозрачно переключает трафик на следующий ближайший. Встроенный SLA 100% uptime. |
| **L4 Balancer (LVS)** | Active-Passive пара на регион, формула N+1. Keepalived с VRRP-протоколом для failover за 1–3 секунды. ECMP-маршрутизация позволяет нескольким активным LVS-узлам делить нагрузку. |
| **L7 Balancer (Nginx)** | Stateless-пул из 6+ узлов на регион, формула N+1. При падении узла LVS перестаёт направлять на него новые соединения после health-check fail. Существующие соединения корректно завершаются на оставшихся узлах. |
| **API Gateway** | Stateless, минимум 10 реплик на регион в Kubernetes, формула N+1. HPA (Horizontal Pod Autoscaler) по CPU и RPS. PodDisruptionBudget гарантирует, что во время rolling update минимум N-1 реплик остаются доступными. |
| **Auth Service / Subscribe Service / File Service / Search Service / Chat Service** | Stateless-сервисы, 3+ реплики каждого, разнесены по разным availability zones внутри региона. Формула N+1. Health-checks `/healthz` и `/readyz` для Kubernetes liveness/readiness probes. |
| **PostgreSQL** (все 5 инстансов) | Мастер + 1 синхронная реплика + 1 асинхронная реплика, формула N+2. Patroni + etcd для автоматического failover за 10–30 секунд. Синхронная реплика подтверждает write до ack клиенту — потеря данных при падении мастера исключена. PITR-бэкапы в S3 ежедневно с retention 30 дней. |
| **Cassandra (`chat_keyspace`)** | Multi-master, Replication Factor 3 по схеме `NetworkTopologyStrategy` (по реплике в каждом из 3 rack'ов), формула RF=3. Запись на QUORUM (2 из 3 реплик) — переживает падение одного узла без потери данных. Hinted handoff и read repair восстанавливают консистентность асинхронно. |
| **Redis** (все 5 инстансов) | Redis Cluster: 3 master-узла + 3 replica-узла минимум, формула N+1 на каждый из 16384 hash slots. Sentinel-протокол кластера выбирает новый master при сбое за 5–15 секунд. Допустима потеря последних миллисекунд write'ов — приемлемо для кешей. |
| **ClickHouse (`dwh_db`)** | ReplicatedMergeTree с RF=2 на каждый шард, координация через ClickHouse Keeper (встроенный Raft), формула N+1. Distributed-таблица поверх локальных шардов делает репликацию прозрачной для приложений. |
| **Qdrant (`vector_index`)** | Replication Factor 2 на каждый shard, Raft consensus для консистентности HNSW-индексов между репликами, формула N+1. Чтения автоматически распределяются между репликами. |
| **Elasticsearch (`fulltext_index`)** | 1 primary + 1 replica на каждый shard, 3 master-eligible узла для quorum при выборах, формула N+1. Cross-zone allocation awareness гарантирует, что primary и replica не на одном узле/зоне. |
| **S3 (оба bucket'а)** | Cross-region replication между 3 регионами (US, EU, APAC), формула N+2. Внутри одного региона S3 даёт durability 99,999999999% (11 девяток) за счёт хранения на 3+ AZ. Versioning включён для защиты от случайного удаления. |
| **Kafka (event backbone)** | `replication.factor=3`, `min.insync.replicas=2`, брокеры разнесены по 3 ДЦ, формула N+1. Producer'ы с `acks=all` ждут подтверждения от 2 ISR-реплик до ack клиенту. Переживает падение одного брокера без потери сообщений. |
| **Inference Scheduler** | Active-active, минимум 3 instance на регион, stateless, формула N+1. Loss of one instance просто снимает 1/N трафика без какого-либо переключения. |
| **vLLM GPU Pools** | Минимум 2 deployment'а на каждую production-версию модели + резерв 20% мощности, формула N+1 группами по 8 GPU (один узел). При падении узла Scheduler перенаправляет трафик на оставшиеся pod'ы того же deployment'а; KV-cache потерянных запросов восстанавливается через retry. |
| **Async Workers** (DWH Ingester, Search Indexer, AV Scanner, Embedding Worker, Text Extractor, Chat Summarizer, Billing Aggregator) | 2+ реплики каждого, Kafka consumer groups автоматически перебалансируют партиции при падении worker'а. Формула N+1. |
| **Singleton Workers** (Quota Flusher) | 2 экземпляра с leader election через etcd, активен только один, второй standby. Re-election за 3–5 секунд при падении leader'а. Формула N+1. |
| **Training Scheduler + Fine-tune Workers** | 2 реплики Scheduler'а с leader election. Fine-tune Workers stateless, могут быть прерваны и перезапущены с checkpoint'а — обучение долгое, потеря 1 часа работы приемлема. Формула N+1. |
| **Model Registry** | 3 реплики stateless-сервиса, формула N+1. Данные в PG защищены своей репликацией. |
| **Evaluation Suite** | Stateless batch-сервис, не критичен к падению (job просто перезапускается). Формула N+1. |
| **Prometheus** | 2 независимых экземпляра, каждый scrape'ит все таргеты независимо, формула N+1 (active-active). Долговременное хранение через remote_write в Thanos/Mimir с S3-backend. |
| **Grafana** | 2 реплики stateless, формула N+1. Дашборды хранятся в PostgreSQL с своей репликацией. |
| **Jaeger** | Collector'ы stateless (3 реплики, N+1), storage backend — Cassandra или Elasticsearch с собственной репликацией. |

### 9.2 Graceful Shutdown

При деплое новой версии, плановом перезапуске или scale-down событии под не может быть просто убит — это приведёт к разорванным in-flight запросам, потерянным SSE-стримам и накоплению ошибок в логах. Graceful shutdown — это контролируемое завершение работы, при котором сервис успевает: (1) отказаться от приёма новых запросов; (2) завершить уже принятые; (3) корректно закрыть соединения с зависимостями.

### Общий протокол shutdown для stateless-сервисов (Go)

Kubernetes отправляет SIGTERM поду и ждёт `terminationGracePeriodSeconds` (по умолчанию у куба это 30 секунд) перед SIGKILL. Сервис обрабатывает сигнал в следующей последовательности:

1. **Фаза 1**. Сервис переводит `/readyz` endpoint в статус NOT_READY. Kubernetes load balancer перестаёт направлять на под новые запросы. При этом liveness-проба продолжает возвращать OK, чтобы под не был убит как сбойный.
2. **Фаза 2**. Сервис продолжает обрабатывать уже принятые запросы. HTTP-сервер вызывает `Server.Shutdown(ctx)` с timeout'ом, который не пускает новые соединения, но даёт текущим завершиться. Для SSE-соединений (Chat Service) — отправляется специальное событие `connection_closing`, и клиент инициирует повторное подключение к другому поду.
3. **Фаза 3**. Закрываются пулы соединений к PostgreSQL, Redis, Cassandra. Commits offset'ов в Kafka consumer groups для read-only consumer'ов. Финальный flush метрик в Prometheus.
4. **Фаза 4**. Процесс завершается с кодом 0.

### Специальные случаи

| Сервис | Особенности graceful shutdown |
|--------|-------------------------------|
| **Chat Service** | SSE-соединения активны минутами. При shutdown отправляется событие `stream_paused` с `retry_after` — клиентский JS переподключается к другому поду через layer-7 балансировщик, и SSE возобновляется. Состояние стрима восстанавливается из Redis pub/sub (канал переживает падение пода). |

## 9.3 Graceful Degradation

При перегрузке или частичном сбое система не должна отказывать пользователям полностью — она должна постепенно отключать или упрощать второстепенный функционал, сохраняя core-функцию (получение ответа на сообщение). Это принцип graceful degradation. Ниже — уровни деградации в порядке нарастания нагрузки или серьёзности сбоя.

### Деградация по уровню нагрузки на GPU pool

| Уровень загрузки | Триггер | Действие | Влияние на пользователя |
|------------------|---------|----------|-------------------------|
| **0–70%** | — | Нормальная работа всех фич | Все возможности доступны |
| **70–85%** | `gpu_pool_utilization > 0.8` | Отключается thinking/reasoning mode для бесплатного тарифа (5% трафика); пользователь видит уведомление "Thinking mode временно недоступен" | Видно только free-tier пользователям с thinking |
| **85–90%** | `gpu_pool_utilization > 0.85` | Отключается web search feature; модель отвечает на основе только своих знаний | Уведомление "Поиск временно недоступен" |
| **90–95%** | `gpu_pool_utilization > 0.9` | Throttle free-tier: возврат HTTP 429 с `Retry-After`; приоритет paid users | Бесплатные пользователи видят сообщение "Высокая нагрузка, попробуйте через минуту"; платные не замечают |
| **95–100%** | `gpu_pool_utilization > 0.95` | Все запросы попадают в очередь с estimated wait time, отображаемым в UI | Пользователи видят "Ваше место в очереди: №247, ожидание ~30 сек" |
| **>100% sustained** | Автомасштабирование не справляется | Включается circuit breaker: новые free-tier запросы отклоняются с HTTP 503 | Бесплатные пользователи получают отказ; платные — в очередь |

### Деградация при сбоях зависимостей

| Сбой | Поведение системы | Влияние на пользователя |
|------|-------------------|-------------------------|
| **Redis `auth_cache` недоступен** | Auth Service fallback на прямой запрос в PostgreSQL `auth_db`. Latency проверки сессии вырастает с 1 мс до 10–20 мс. | Незаметно для пользователя |
| **Redis `quota_counters` недоступен** | Rate-limiting временно работает через прямой запрос в PG `usage_quotas` (lag до 30 секунд по последнему flush). Возможен временный over-limit на единицы процентов. | Незаметно |
| **Redis `chat_cache` недоступен** | Стриминг токенов переключается на fallback через `topic: token_stream_fallback` в Kafka. Latency time-to-first-token вырастает с 200 мс до 500–700 мс. | Заметна более долгая задержка перед началом ответа |
| **Elasticsearch недоступен** | Search Service возвращает только semantic результаты из Qdrant. Recall по точным совпадениям ключевых слов снижается. | Поиск работает, но менее точно для exact match |
| **Qdrant недоступен** | Search Service возвращает только full-text результаты из Elasticsearch. Семантический поиск отключён. | Поиск работает, но не находит синонимы |
| **И Qdrant, и Elasticsearch недоступны** | Search Service deg mode — поиск только по `chats.title` через прямой запрос в Cassandra. Покрывает ~10% реальных поисковых запросов. | Большинство поисковых запросов возвращают "ничего не найдено" |
| **PostgreSQL master недоступен** | Failover на синхронную реплику через Patroni за 10–30 секунд. В течение failover'а write-операции возвращают HTTP 503. | Кратковременная (10–30 сек) недоступность регистрации, оплаты; чат продолжает работать (Cassandra независима) |
| **Kafka недоступна (полностью)** | Chat Service пишет события в локальный файл fallback'а. После восстановления — асинхронный replay. Inference продолжает работать через прямой gRPC вызов Scheduler'а в обход очереди (degraded mode). | Аналитика отстаёт; биллинг отстаёт; пользователь не замечает |
| **Model Registry недоступен** | Inference Scheduler использует последний закешированный конфиг из Redis `model_cache`. Новые версии моделей не выкатываются, но существующие работают. | Незаметно |
| **vLLM Pool падает полностью** | Inference Scheduler переключает 100% трафика на резервный pool. Качество ответов может снизиться (если переключение на более простую модель). | Возможно более простые ответы |

### Принципы реализации деградации

- **Feature flags в Redis**: каждая фича (search, thinking, vision) имеет глобальный флаг enabled/disabled, который Chat Service проверяет перед использованием. Управляется автоматически на основе Prometheus метрик через AlertManager → automation hook.
- **Circuit breakers**: каждый downstream-вызов обёрнут в circuit breaker (библиотека `gobreaker`). После 5 последовательных failures circuit открывается на 30 секунд, в течение которых все вызовы fail-fast без реальной попытки — это спасает от каскадного падения.
- **Bulkheads**: пулы соединений к каждой downstream-системе изолированы. Зависание PostgreSQL не съест все горутины Chat Service'а — пул PG ограничен 50 соединениями, после чего новые запросы получают error fast.
- **Retry с exponential backoff и jitter**: при transient-ошибках retry, но с экспоненциальной задержкой и случайным jitter'ом для избежания thundering herd при восстановлении сервиса.
- **Timeouts на все вызовы**: ни один HTTP/gRPC/DB вызов не делается без явного timeout'а. Default — 5 секунд, для inference — 60 секунд.
   ## 10. Cхема проекта
